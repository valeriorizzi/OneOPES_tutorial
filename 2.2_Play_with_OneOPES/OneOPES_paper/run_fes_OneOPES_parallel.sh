...
compute_fes() {
  cd $i
  rm fes-*
  python ../../FES_from_Reweighting_multiT_funnel.py --skip 25000 --sigma 0.05 --colvar COLVAR.* --cv phi --bin 150 --temp 300 --min -3 --max 2 --deltaFat -0.2 --stride 5000; cd .. ; grep 'DeltaF' $i/fes-* | awk '{print $4}' > $i/deltaF.dat 
}
